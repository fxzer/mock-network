...
async function toggleIframeVisibility() {
  try {
    const { iframeVisible } = await chrome.storage.local.get({
      iframeVisible: true,
    });
    ...
    // 检查 response 是否有效
    if (response && typeof response.nextIframeVisible !== 'undefined') {
      await chrome.storage.local.set({
        iframeVisible: response.nextIframeVisible,
      });
    }
  } catch (error) {
    console.error('toggleIframeVisibility error:', error);
  }
}
function setSwitchBadge(switchValue) {
  chrome.action.setBadgeText({ text: switchValue ? 'ON' : 'OFF' });
  chrome.action.setBadgeTextColor({
    color: switchValue ? '#ffffff' : '#333333',
  });
  chrome.action.setBadgeBackgroundColor({
    color: switchValue ? '#4480f7' : '#bfbfbf',
  });
}
